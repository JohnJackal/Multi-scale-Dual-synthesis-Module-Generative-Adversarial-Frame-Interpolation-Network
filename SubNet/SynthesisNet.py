...
# define the synthesis subnet that has skip connection
class SynthesisSkip(nn.Module):
    r"""
    Args:
    ...
    def __init__(self,
        ...
        super(SynthesisSkip, self).__init__()
...
# define the synthesis subnet that has skip connection
class SynthesisNoskip(nn.Module):
    r"""
    Args:
        growth_rate (int) - how many filters to add each layer (`k` in paper)
        block_num (int) - how many dense blocks there are.
    ...
    def __init__(self,
        ...
        super(SynthesisNoskip, self).__init__()

        # 1st conv layer (with filter size 5*5)
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv2d(3, growth_rate, kernel_size=5, stride=1,
                                padding=2, bias=False)),
            ('norm0', nn.BatchNorm2d(growth_rate)),
            ('mish0', DB.Mish()),
        ]))

        # Each DenseBlock has 3 conv layers, 3 bn layers and 2 activation functions.
        num_features = growth_rate # num of feature maps
        for i in range(block_num):
            block = DB.DenseBlock(
                num_layers=3,
                num_input_features=num_features,
